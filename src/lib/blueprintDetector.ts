...
export async function detectBlueprints(marketData: MarketData[]): Promise<BlueprintResult[]> {
...
function analyzeSymbol(data: MarketData): BlueprintResult[] {
  ...
  if (detectRejectionDay(data, range, upperWick, lowerWick)) {
    ...
    results.push({
      symbol: data.symbol,
      blueprintType: data.change24h > 0 ? "Long Rejection Day" : "Short Rejection Day",
      confidence,
      price: data.price,
      change24h: data.change24h,
      volume: data.volume,
      details: `${data.change24h > 0 ? "Bullish" : "Bearish"} rejection with ${((upperWick + lowerWick) / range * 100).toFixed(1)}% wick ratio`
    });
  }
  ...
  if (detectFailedNewHigh(data, changePercent)) {
    ...
    results.push({
      ...
      change24h: data.change24h,
      volume: data.volume,
      details: `Failed to sustain new highs, showing weakness with ${changePercent.toFixed(2)}% reversal`
    });
  }
  ...
  if (detectFailedNewLow(data, changePercent)) {
    ...
    results.push({
      ...
      change24h: data.change24h,
      volume: data.volume,
      details: `Failed to sustain new lows, showing strength with ${changePercent.toFixed(2)}% recovery`
    });
  }
  ...
  if (detectOutsideDay(data, range, changePercent)) {
    ...
    results.push({
      symbol: data.symbol,
      blueprintType: data.change24h > 0 ? "Bullish Outside Day" : "Bearish Outside Day",
      confidence,
      price: data.price,
      change24h: data.change24h,
      volume: data.volume,
      details: `High volatility outside day with ${changePercent.toFixed(2)}% move`
    });
  }
  ...
  if (detectAbsorptionDay(data, bodySize, range, data.volume)) {
    ...
    results.push({
      symbol: data.symbol,
      blueprintType: data.change24h > 0 ? "Bullish Absorption Day" : "Bearish Absorption Day",
      confidence,
      price: data.price,
      change24h: data.change24h,
      volume: data.volume,
      details: `High volume absorption with ${(bodySize / range * 100).toFixed(1)}% body ratio`
    });
  }
  ...
  if (detectStopRunDay(data, upperWick, lowerWick, range)) {
    ...
    results.push({
      symbol: data.symbol,
      blueprintType: upperWick > lowerWick ? "Stop Run Day High" : "Stop Run Day Low",
      confidence,
      price: data.price,
      change24h: data.change24h,
      volume: data.volume,
      details: `Stop run detected with ${((Math.max(upperWick, lowerWick)) / range * 100).toFixed(1)}% wick`
    });
  }
...
function detectRejectionDay(data: MarketData, range: number, upperWick: number, lowerWick: number): boolean {
...
function detectOutsideDay(data: MarketData, range: number, changePercent: number): boolean {
  return changePercent > 8 && range / data.price > 0.05;
}

function detectAbsorptionDay(data: MarketData, bodySize: number, range: number, volume: number): boolean {
...
function detectStopRunDay(data: MarketData, upperWick: number, lowerWick: number, range: number): boolean {
